...
function requireTransitionTarget(args: Record<string, any>) {
  ...
  if (
    ![trackId, leftClipId, rightClipId, seamTime].every(
      (value) => typeof value === 'string' && value,
    )
  ) {
    throw toolError('invalid_arguments', 'trackId、leftClipId、rightClipId 和 seamTime 为必填项。')
  }
  if (!isValidAgentToolTimecode(seamTime))
    throw toolError('invalid_timecode', 'seamTime 必须是 HH:MM:SS+FF 时间码。')
  const store = useUnifiedStore()
  const track = store.getTrack(trackId)
  if (!track || track.type !== 'video')
    throw toolError('invalid_transition_target', '目标必须是存在的视频轨道。')
  ...
  if (
    !left ||
    !right ||
    left.trackId !== trackId ||
    right.trackId !== trackId ||
    !getTrackItems(trackId).includes(left) ||
    !getTrackItems(trackId).includes(right)
  ) {
    throw toolError('invalid_transition_target', '左右 clip 必须属于目标轨道。')
  }
  if (
    left.timeRange.timelineEndTime !== seamFrame ||
    right.timeRange.timelineStartTime !== seamFrame
  ) {
    throw toolError(
      'invalid_transition_target',
      'seamTime 必须同时等于左 clip 的结束和右 clip 的开始。',
    )
  }
  const neighbors = getTrackItems(trackId).filter(
    (item) => item.timeRange.timelineStartTime === seamFrame,
  )
...
export async function executeSearchTransitions(args: Record<string, any>) {
  if (typeof args.query !== 'string' || !args.query.trim())
    return buildToolError('search_transitions', 'invalid_arguments', 'query 必须是非空字符串。')
  try {
...
export async function executeApplyTransition(args: Record<string, any>) {
  try {
    ...
    const preview = {
      ...target.left,
      exRenderConfig: { ...target.left.exRenderConfig, transition: config },
    }
    const snapshot = getTransitionSnapshot(preview)
    if (!snapshot || transitionOverlaps(snapshot, target.left.id))
      throw toolError('transition_overlap', '转场有效范围会与共享 clip 上的已有转场重叠。')
    ...
    return buildToolSuccess('apply_transition', {
      ...
      item: created && {
        ...transitionTimelineOutput(created),
        transition: {
          templateId: config.templateId,
          duration: framesToTimecode(config.durationFrames),
...
export async function executeRemoveItem(args: Record<string, any>) {
  ...
  if (
    ...
    !itemIds.length ||
    !itemIds.every((id) => typeof id === 'string' && id)
  )
    return buildToolError('remove_item', 'invalid_arguments', 'itemIds 必须是非空字符串数组。')
    ...
    for (const itemId of normalizedIds) {
      ...
      if (!parsed) {
        if (itemId.startsWith('transition:'))
          throw toolError('invalid_transition_item', `无效转场 itemId: ${itemId}`)
        if (!store.getTimelineItem(itemId))
          throw toolError('item_not_found', `未找到 clip: ${itemId}`)
        clipIds.push(itemId)
        continue
      }
      ...
      if (snapshot.config.templateId !== parsed.templateId)
        throw toolError('stale_transition_item', '转场模板已被替换，拒绝删除新的转场。')
      transitionItems.push(snapshot)
    }
    const batch = store.startBatch(historyLabels.deleteTimelineItems(normalizedIds.length))
    for (const transition of transitionItems) {
      if (!transition) continue
      batch.addCommand(
        new UpdateTransitionConfigCommand(transition.leftClipId, transition.config, undefined, {
          getTimelineItem: (id) => store.getTimelineItem(id),
          setTimelineItemTransitionConfigForCmd:
            store.setTimelineItemTransitionConfigForCmd.bind(store),
        }),
      )
    }
...
export async function executeReadTransitionItem(itemId: string, propertyGroups: unknown) {
  ...
  if (snapshot.config.templateId !== parsed.templateId)
    throw toolError('stale_transition_item', '转场模板已被替换。')
  if (
    !Array.isArray(propertyGroups) ||
    !propertyGroups.length ||
    propertyGroups.some((group) => group !== 'timeline' && group !== 'transition')
  )
    throw toolError('invalid_property_group', '转场只支持 timeline 和 transition 属性组。')
  const groups: Record<string, unknown> = {}
  if (propertyGroups.includes('timeline')) groups.timeline = transitionTimelineOutput(snapshot)
  if (propertyGroups.includes('transition'))
    groups.transition = {
...
export async function executeUpdateTransitionItem(itemId: string, match: unknown, apply: unknown) {
  ...
  if (
    ...
    typeof apply !== 'object' ||
    Array.isArray(apply) ||
    !Object.keys(apply).length
  )
    throw toolError('invalid_arguments', 'match 和 apply 必须是非空对象。')
  ...
  for (const key of Object.keys(apply as Record<string, unknown>)) {
    const current =
      key === 'transition.duration'
        ? framesToTimecode(snapshot.config.durationFrames)
        : key.startsWith('transition.params.')
          ? snapshot.config.params[key.slice('transition.params.'.length)]
          : undefined
    if (JSON.stringify(current) !== JSON.stringify((match as Record<string, unknown>)[key]))
      throw toolError('match_failed', `属性匹配失败: ${key}`, {
        expected: (match as Record<string, unknown>)[key],
        actual: current,
      })
    if (key === 'transition.duration') {
      ...
      if (typeof value !== 'string' || !isValidAgentToolTimecode(value))
        throw toolError('invalid_timecode', 'transition.duration 必须是 HH:MM:SS+FF。')
      next.durationFrames = parseAgentToolTimecode(value)
      if (next.durationFrames < 2)
        throw toolError('invalid_transition_duration', 'transition.duration 至少为 2 帧。')
      ...
      next.params[parameterKey] = validateTransitionParam(
        (apply as Record<string, unknown>)[key],
        definition,
        key,
      )
  ...
  const preview = {
    ...snapshot.leftItem,
    exRenderConfig: { ...snapshot.leftItem.exRenderConfig, transition: next },
  }
  const nextSnapshot = getTransitionSnapshot(preview)
  if (!nextSnapshot || transitionOverlaps(nextSnapshot, snapshot.leftClipId))
    throw toolError('transition_overlap', '新的转场时长会与共享 clip 上的已有转场重叠。')
...
export const searchTransitionsTool: ToolDefinition = {
  name: 'search_transitions',
  execute: executeSearchTransitions,
} as ToolDefinition
export const applyTransitionTool: ToolDefinition = {
  name: 'apply_transition',
  execute: executeApplyTransition,
} as ToolDefinition
export const removeItemTool: ToolDefinition = {
  name: 'remove_item',
  execute: executeRemoveItem,
} as ToolDefinition
