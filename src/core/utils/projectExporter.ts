    ...
    await Promise.all(
      this.clonedTimelineItems.map(async (item) => {
        ...
        if (
          ...
          const isItemMuted = TimelineItemQueries.getAudioRenderConfig(item).isMuted ?? false
          ...
          if (state === 'success') {
            ...
            if (shouldRequestAudio && audio && audio.length > 0) {
              ...
              const audioConfig = TimelineItemQueries.getAudioRenderConfig(item, config)
              const currentVolume = audioConfig.volume ?? 1.0
    ...
    await Promise.all(
      this.clonedTimelineItems.map(async (item) => {
        ...
        if (
          ...
          const track = this.config.tracks.find((t) => t.id === item.trackId)
          const isTrackMuted = track?.isMuted ?? false
          const baseAudioConfig = TimelineItemQueries.getAudioRenderConfig(item)
          const isItemMuted = baseAudioConfig.isMuted ?? false
          const shouldRequestAudio = !isTrackMuted && !isItemMuted
