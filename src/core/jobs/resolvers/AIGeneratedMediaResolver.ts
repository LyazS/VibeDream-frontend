...
export const REMOTE_TASK_SUBMITTED_RESOURCE_TYPE = 'remote-task-submitted'
  ...
  implements ResourceResolver<AIGeneratedMediaInput, AIGeneratedMediaResult>
{
  ...
  async resolve(ctx: ResolveContext<AIGeneratedMediaInput>): Promise<AIGeneratedMediaResult> {
    ...
    const taskId =
      getTaskId(source) ||
      (
        await ctx.ensure<RemoteTaskSubmittedResult>(
          createRemoteTaskSubmittedRequest(mediaItem.id, provider),
        )
      ).taskId
...
export class RemoteTaskSubmittedResolver
  implements ResourceResolver<RemoteTaskSubmittedInput, RemoteTaskSubmittedResult>
{
  readonly type = REMOTE_TASK_SUBMITTED_RESOURCE_TYPE
...
export function createRemoteTaskSubmittedResolver(
  mediaModule: AIGeneratedMediaModule,
): RemoteTaskSubmittedResolver {
  return new RemoteTaskSubmittedResolver(mediaModule)
}
...
export function createRemoteTaskSubmittedRequest(
  mediaId: string,
  provider: AIGeneratedMediaProvider,
  policy?: ResourcePolicy,
): ResourceRequest<RemoteTaskSubmittedInput> {
  return {
    type: REMOTE_TASK_SUBMITTED_RESOURCE_TYPE,
