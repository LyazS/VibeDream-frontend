    ...
    try {
      ...
      for (const item of splitItems) {
        ...
        if (TimelineItemQueries.isLoading(item)) {
          console.log('🔗 [SplitTimelineItemCommand] trigger timeline-item-ready', {
            timelineItemId: item.id,
            mediaItemId: item.mediaItemId,
            isInitialized: item.runtime.isInitialized,
            timelineStatus: item.timelineStatus,
          })
          void this.ensureTimelineItemReady(item.id).catch((error) => {
            console.error(`❌ timeline-item-ready 启动失败: ${item.id}`, error)
          })
        }
      }
  ...
  async undo(): Promise<void> {
    try {
      ...
      if (TimelineItemQueries.isLoading(originalItem)) {
        ...
        void this.ensureTimelineItemReady(originalItem.id).catch((error) => {
          console.error(`❌ timeline-item-ready 启动失败: ${originalItem.id}`, error)
        })
      }
