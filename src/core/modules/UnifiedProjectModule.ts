...
export function createUnifiedProjectModule(registry: ModuleRegistry) {
  ...
  let ensureTimelineItemReadyForProjectLoad: ((timelineItemId: string) => Promise<unknown>) | null =
    null
  ...
  function setTimelineItemReadyEnsurer(
    ensurer: (timelineItemId: string) => Promise<unknown>,
  ): void {
    ensureTimelineItemReadyForProjectLoad = ensurer
  }
        ...
        for (const itemData of savedTimelineItems) {
          try {
            ...
            if (
              ...
              ensureTimelineItemReadyForProjectLoad
            ) {
              console.log('🔗 [restoreTimelineItems] trigger timeline-item-ready', {
              ...
              void ensureTimelineItemReadyForProjectLoad(newTimelineItem.id).catch((error) => {
                console.error(
                  `❌ [restoreTimelineItems] timeline-item-ready 启动失败: ${newTimelineItem.id}`,
                  error,
                )
              })
            }
